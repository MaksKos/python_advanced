...
class TestCline(TestCase):
    ...
    @mock.patch('client.threading')  
    def test_main_file(self, mock_thr):
        n_thread = 5
        file_name = 'url.txt'
    ...
    def test_thread_socket(self, mock_print, mock_sock):
        ...
        lock = mock.Mock()
        sock_s.recv.side_effect = [(val+'\n').encode() for val in self.urls.split('\n')]
        send = [mock.call((val+'\n').encode()) for val in self.urls.split()]
...
class TestServer(TestCase):
    ...
    def test_main(self, queue, mock_work, mock_mast):
        ...
        q = queue.Queue()
        args_work = mock.call(q, self.n_top)
        args_mast = mock.call(q)
    ...
    def test_master(self):
        ...
        master.join()
        self.assertTrue(isinstance(q.get(), socket.socket))
    ...
    def test_worker(self, req):
        ...
        q = queue.Queue()
        master = server.Master(q)
        worker = server.Worker(q, self.n_top)
        master.setDaemon(True)
        ...
        self.assertEqual(req.get.call_args_list, [mock.call(urls[0], allow_redirects=False),
