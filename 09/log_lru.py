...
class Node:
    ...
    def __init__(self, key, value, loglru):
        self.key = key
        self.value = value
        self.prev = None
        self.next = None
...
class LRUCache:
    ...
    def __init__(self, logger, limit=42):
        ...
        self.limit = limit
        self.loglru = logger
        self.loglru.info('create lru with size %d', limit)

    def _appnode(self, new_node):
        self.loglru.debug('add node with key <%s>', new_node.key)
    ...
    def _removenode(self, node):
        self.loglru.debug('remove node with key <%s>', node.key)
        node_next = node.next
        node_prev = node.prev
    ...
    def _move_to_head(self, node):
        self.loglru.debug('move to head node with key <%s>', node.key)
    ...
    def get(self, key):
        ...
        if node is None:
            self.loglru.critical('get: key <%s> not found', key)
            return None
        self.loglru.info('get: key <%s> is found', key)
    ...
    def set(self, key, value):
        node = self.cache.get(key)
        if node is None:
            self.loglru.warning('set: add new key <%s>', key)
            new_node = Node(key, value, self.loglru)
            self.cache[key] = new_node
            self._appnode(new_node)
        else:
            self.loglru.info('set: update key <%s>', key)
            node.value = value
            self._move_to_head(node)

        if len(self.cache) > self.limit:
            self.loglru.error('set: delete key <%s>', self.tail.key)
...
def main():
    ...
    args = get_args()
    logger = logger_config(args.f, args.s)

    lru_cache = LRUCache(logger, 3)
